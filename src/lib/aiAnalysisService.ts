...
export function generateFallbackAnalysis(payload: AIAnalysisPayload): AIAnalysisData {
  ...
  return {
    ...
    instagram_summary: `تم فحص حساب الإنستغرام (@${instaUser}) الخاص بنَسْجَة؛ المحتوى البصري يعكس أصالة وجودة الأقمشة البحرينية والخليجية ويشكل نافذة جذب رئيسية لطلبات التفصيل.`,
    ...
    instagram_promotion_advice: fabricsCount > 0
      ...
      : "إعداد حملة تصوير لأحدث الأقمشة الرجالية مع رابط محادثة واتساب مباشر لتسريع تلقي طلبات تفصيل الثياب.",
    instagram_profile: {
      username: instaUser,
